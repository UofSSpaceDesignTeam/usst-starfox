...
import math
from tkinterUI import tkinter_starter 
...
UI_Thread = tkinter_starter(Motor_Thread)
...
def main():
    ...
    rpm_unrounded=Motor_values["RPM"]/(19*12)
    rpm1=("{0:.2f}".format(rpm_unrounded))
    ...
    y=LSMC_values["y"]
    z=LSMC_values["z"]
    magnitude_unrounded=math.sqrt(z*z+x*x+y*y)
    magnitude=("{0:.2f}".format(magnitude_unrounded))


    to_save.write(str(time_keep))
    to_save.write(',       {0:3.3f},                {1:3.3f},               {2:3.3f},               {3:3.3f},               {4:3.3f},               {5:3.3f},               {6:3.3f},               {7:3.3f},               {8:3.3f},               {9:3.3f}                 '.format(degrees, kilopascals, humidity,x,y,z,rpm_unrounded,tachometer,watts,current))
    to_save.write('\n')
    
    
    data={"rpm":rpm1,"acceleration":magnitude,"Temperature":degrees,"Humidity":humidity,"Pressure":kilopascals}
    UI_Thread.app.data_from_app = data
...
try:
    ...
    UI_Thread.app.main = main
    
    #UI_Thread.app.root.after(1000, UI_Thread.app.main)
    #UI_Thread.app.root.after(1000, UI_Thread.app.starting)
    #UI_Thread.app.root.after(1000, UI_Thread.app.ui_showdata)
    
    UI_Thread.app.starting()
    while True:
        try:
            
            UI_Thread.app.main()
            
            UI_Thread.app.ui_showdata()
            UI_Thread.root.update()
